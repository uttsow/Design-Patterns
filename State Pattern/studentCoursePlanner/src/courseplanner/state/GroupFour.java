...
public class GroupFour implements CoursePlannerStateI{
    ...
    private final char START = 'M';
    private final char END = 'P';

    /*
    Constructor
    */
    public GroupFour(CoursePlannerContext contextIn){
    ...
    */
    public boolean takeCourse(String courseIn){
        // Debug.print("{STATE4}Attempting to take course from State 4: " + courseIn);
        String coursePeek = courses.peek();
        if(courseIn.equals(coursePeek) && (preReqTaking == false)){
            // Debug.print("{STATE4}TOOK COURSE from State 4: " + courses.peek());
            courses.poll();
            tookPre(true);
    ...
    */
    public void incrementCoursesTaken(){
        // Debug.print("INCREMENTING STATE 4 Course Counter");
        courseCounter += 1;
        if(courseCounter > context.getState().getCourseTakenCounter()){
            // Debug.print("{STATE4} STATE CHANGE!");
            context.setState(context.getG4());
        }
    ...
    */
    public void setGroupReq(boolean standingIn){
        Debug.print("{STATE4} Group req fufilled!");
